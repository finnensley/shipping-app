...
const InventoryPage = () => {
  ...
  // Refetch inventory data when page regains focus
  useEffect(() => {
    const handleFocus = async () => {
      const response = await fetch(`${API_URL}/items`);
      const freshData = await response.json();
      if (freshData && freshData.items) {
        dispatch(setInventory(freshData.items));
      }
    };
                        ...
                        <div className="flex flex-col items-center gap-2">
                          {(item.locations || []).map((location) => (
                            <input
                              ...
                              } // ?? is nullish coalescing operator. if left side is null or undefined use right side
                              min={0}
                              onChange={(e) => {
                                setQuantities((q) => ({
