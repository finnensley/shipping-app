...
describe("Pick List Creation Workflow", () => {
  ...
  beforeEach(async () => {
    ...
    await testPool.query(
      "TRUNCATE TABLE picked_orders_staged_for_packing, order_items, orders, item_location_history, item_locations, locations, items, customers RESTART IDENTITY CASCADE",
    );
  ...
  test("should handle inventory transfer during pick list creation", async () => {
    ...
    expect(itemResult.rows[0].total_quantity).toBe(48);
  });
});
